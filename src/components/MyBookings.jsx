...
const MyBookings = () => {
  ...
  const handleSelectUser = async selectedUserData => {
    try {
      ...
      console.log(
        'ユーザー:',
        `${selectedUserData.companyName} ${selectedUserData.fullName}`
      );
      ...
      allCourses.forEach(course => {
        course.schedules?.forEach(schedule => {
          ...
          const counts = bookingCounts[key] || {
            totalBookings: 0,
            pcRentals: 0,
          };
          ...
          // このユーザーが既にこの具体的なスケジュールに申し込んでいるかチェック
          const isAlreadyBooked = myBookingsList.some(
            booking =>
      ...
      allSchedulesList.sort((a, b) =>
        dayjs(a.schedule.dateTime.toDate()).diff(
          dayjs(b.schedule.dateTime.toDate())
        )
      );
      ...
      if (err.code === 'permission-denied') {
        errorMessage =
          'データベースへのアクセス権限がありません。管理者にお問い合わせください。';
  ...
  const handleCancelConfirm = async () => {
    ...
    try {
      ...
      // 成功メッセージを表示
      setSuccessMessage(
        '申し込みをキャンセルしました。同じ講座の別の日時や他の講座に申し込むことができます。'
      );
      setError(null);
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {searchResults.map(userData => (
                        <TableRow
                          key={`${userData.companyName}-${userData.fullName}`}
                          hover
                        >
                          <TableCell>
        ...
        <>
          ...
          <Card sx={{ mb: 4 }}>
            <CardContent>
              ...
              {myBookings.length === 0 ? (
                <Alert severity='info'>
                  申し込み履歴がありません。下記の「全講座一覧」から新しく申し込みを行ってください。
                </Alert>
              ) : (
                <TableContainer component={Paper} variant='outlined'>
                  <Table>
                      ...
                      {myBookings.map(booking => {
                        ...
                        return (
                          <TableRow key={booking.id}>
                            <TableCell>
                              {dayjs(booking.createdAt.toDate()).format(
                                'YYYY/MM/DD HH:mm'
                              )}
                            </TableCell>
                            <TableCell>
                            ...
                            <TableCell align='center'>
                              <Chip
                                size='small'
                                label={
                                  booking.needsPcRental ? 'PC貸出' : 'PC持参'
                                }
                                color={
                            ...
                            <TableCell align='center'>
                              {isUpcoming ? (
                                <Tooltip title='申し込みをキャンセルします。同じ講座の別日時には追加申し込みも可能です。'>
                                  <IconButton
                                    size='small'
                                    color='error'
          ...
          <Card>
            <CardContent>
              ...
              <Typography variant='body2' color='text.secondary' sx={{ mb: 3 }}>
                すべての講座の一覧です。既に申し込み済みのスケジュールは「申込済み」と表示されます。
                <br />
                同じ講座名でも日時が異なるスケジュールには追加で申し込むことができます。
                <br />
                スケジュールを変更したい場合は、申込履歴からキャンセルしてから新しく申し込みを行ってください。
              </Typography>
              ...
              {otherSchedules.length === 0 ? (
                ...
                <Box>
                  {otherSchedules.map(item => (
                    <Accordion
                      key={`${item.courseId}-${item.schedule.id}`}
                      sx={{ mb: 1 }}
                    >
                        ...
                        >
                          ...
                          <Box display='flex' gap={1} alignItems='center'>
                            ...
                            <Chip
                              ...
                              color={
                                item.pcSlotsAvailable > 0 ? 'info' : 'warning'
                              }
                              variant='outlined'
                              icon={<Computer />}
                            />
                        ...
                        <Grid container spacing={3}>
                          <Grid item xs={12} md={8}>
                            ...
                            <Box sx={{ mb: 2 }}>
                              ...
                              <Typography variant='body2'>
                                <strong>定員:</strong> {item.schedule.capacity}
                                名
                              </Typography>
                              <Typography variant='body2'>
                                <strong>PC貸出枠:</strong>{' '}
                                {item.schedule.pcRentalSlots}台
                              </Typography>
                            </Box>
                          </Grid>
                          ...
                          <Grid item xs={12} md={4}>
                            <Card variant='outlined'>
                              <CardContent>
                                ...
                                <Typography variant='body2'>
                                  参加者: {item.totalBookings}/
                                  {item.schedule.capacity}名
                                </Typography>
                                <Typography variant='body2'>
                                ...
                                {item.isAlreadyBooked ? (
                                  <Box sx={{ textAlign: 'center', py: 2 }}>
                                    ...
                                    <Typography
                                      ...
                                      sx={{ mt: 1 }}
                                    >
                                      この日時は申込済みです。同じ講座の別の日時には追加で申し込めます。
                                      <br />
                                      日時を変更したい場合は、申込履歴からキャンセルしてください。
                                    </Typography>
                                  </Box>
                                ) : (
                                  <Button
                                    ...
                                    onClick={() =>
                                      (window.location.href = `/booking/${item.courseId}`)
                                    }
                                  >
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id='cancel-dialog-description'>
            {bookingToCancel && (
              <>
                ...
                <br />
                <strong>PC貸出:</strong>{' '}
                {bookingToCancel.needsPcRental ? 'PC貸出' : 'PC持参'}
                <br />
                <br />
                キャンセル後、同じ講座の別の日時や他の講座に追加で申し込むことができます。
              </>
            )}
          </DialogContentText>
          ...
          <Box sx={{ mt: 3 }}>
            <TextField
              ...
              helperText={
                cancelReasonError || '5文字以上500文字以内で入力してください'
              }
        ...
        <DialogActions>
          ...
          <Button
            ...
            startIcon={
              cancelLoading ? <CircularProgress size={20} /> : <Cancel />
            }
          >
